...
@Component({
  selector: 'app-audit-pwd-policy',
  templateUrl: './audit-pwd-policy.component.html',
  styleUrls: ['./audit-pwd-policy.component.css']
})
export class AuditPwdPolicyComponent implements OnInit {
  bln_loading: boolean;
  bln_show: boolean = false;
  todayDate = new Date();
  // Declaring Reactive Form
  adminPwdPolicy = new FormGroup({
  ...
  constructor(
    ...
    this.adminPwdPolicy = this.fb.group({
  ...
  addEvent() {
    this.fromDate = this.adminPwdPolicy.value.fromDate;
    const toDate = this.datePipe.transform(
      this.adminPwdPolicy.value.toDate,
      "yyyy/MM/dd"
    );
    const fromDate = this.datePipe.transform(
      this.adminPwdPolicy.value.fromDate,
      "yyyy/MM/dd"
    );
    if (fromDate > toDate) {
      swal("", "FROM DATE should be less than TO DATE", "warning");
      this.adminPwdPolicy.patchValue({
  ...
  onSubmit() {
    ...
    const FileName = "RepoAuditPasswordPolicy";
    const data: Object = {};
    ...
    const finalObject=Object.assign({reportObj:data},{strTableName:"tbl_audit_pwd_complexity"});
    if (fromDate > toDate) {
      swal("", "FROM DATE should be less than TO DATE", "warning");
    } else {
