...
export class MachineComponent implements OnInit {
  ...
  constructor(
    ...
    this.machine = this.fb.group({
      machineID: new FormControl("All", [Validators.required]),
  ...
  //Below funtion will return list of all machine
  getData() {
  ...
  onSubmit() {
    const FileName = "RepoAuditMachine";
    const toDate = this.datePipe.transform(
      this.machine.value.toDate,
      "yyyy/MM/dd"
    );
    ...
    var selectedValue = this.machine.value.machineID;
    if (selectedValue == "All") {
      selectedValue = "";
    }
    var selectedAction = this.machine.value.action;
    ...
    const finalObject=Object.assign({reportObj:data},{strTableName:"tbl_audit_machine"});
    if (fromDate > toDate) {
      swal("", "FROM DATE should be less than TO DATE", "warning");
    } else {
      this.bln_loading = true;
      this.http
        .postMethod("report/rptAuditMachine", finalObject)
        .subscribe((res: any) => {
