...
export class AreaSettingComponent implements OnInit {
  ...
  constructor(
    ...
    this.http.getMethod("area/getAuditArea").subscribe(
      (res: any) => {
  ...
  onSubmit() {
    const FileName = "RepoAuditAreaSetting";
    const toDate = this.datePipe.transform(
      this.areaSetting.value.toDate,
      "yyyy/MM/dd"
    );
    ...
    const finalObject=Object.assign({reportObj:data},{strTableName:"tbl_audit_areasetting"});
    if (fromDate > toDate) {
      swal("", "FROM DATE should be less than TO DATE", "warning");
    } else {
      this.bln_loading = true;
      this.http
        .postMethod("report/rptAuditArea", finalObject)
        .subscribe((res: any) => {
